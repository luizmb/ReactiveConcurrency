...
/// Monadic bind — sequences a dependent effect (container on the left) for the DeferredTask-over-Array stack. Operator form of `flatMap`.
...
/// Monadic bind — sequences a dependent effect (function on the left) for the DeferredTask-over-Array stack. Operator form of `flatMap`.
public func -<< <A: Sendable, B: Sendable>(
    _ fn: @escaping @Sendable (A) -> DeferredTask<[B]>,
    _ task: DeferredTask<[A]>
) -> DeferredTask<[B]> {
...
/// Left-to-right Kleisli composition of two effectful functions for the DeferredTask-over-Array stack.
...
/// Reverse Kleisli composition — `g <=< f == f >=> g` for the DeferredTask-over-Array stack.
public func <=< <A: Sendable, B: Sendable, C: Sendable>(
    _ fn2: @escaping @Sendable (B) -> DeferredTask<[C]>,
    _ fn1: @escaping @Sendable (A) -> DeferredTask<[B]>
) -> @Sendable (A) -> DeferredTask<[C]> {
